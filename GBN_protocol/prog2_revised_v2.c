...
struct pkt B_sndpkt; // retransmit packet


int check_sum(struct pkt* packet) { // 8-bit 1's complement checksum
    unsigned int sum = 0;
    ...
    unsigned char chksum = ~sum; // 1's complement
    return chksum;
}
...
void sndWindow(void) { //send packet to window
    while (A_next_seqnum < A_base + A_windowSize) { // send all packets within Windows
        if (A_next_seqnum < A_next_buff) {
            struct pkt* sndpkt = &A_pkt_buff[A_next_seqnum % MAX_BUFFER_SIZE]; // packet extraction in Windows
            printf("===send packet (seq=%d): %s\n", sndpkt->seqnum, sndpkt->payload);
            tolayer3(0, *sndpkt); // send to network

            if (A_base == A_next_seqnum) { // all packets have been sent, the timer restarts
                starttimer(0, A_timer);
            }
            A_next_seqnum++;
        }
        else break; //Sent all windows
    }
}
...
A_init()
{
    ...
    memset(A_pkt_buff, 0, sizeof(A_pkt_buff)); //clear buffer
}
