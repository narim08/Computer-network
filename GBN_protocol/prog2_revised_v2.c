...
#define MAX_BUFFER_SIZE 64 // Maximum buffer size for sender window
...
struct msg message;
{
    if (A_next_seqnum < A_base + A_windowSize) {
        if (A_ACKstate == 0) {
            A_ACKnum = 999;
        }
        struct pkt packet = make_pkt(message.data, A_next_seqnum, A_ACKnum);
        A_sender_window[A_next_seqnum - A_base] = packet; // Store packet in sender window
...
struct msg message;
{
    if (B_next_seqnum < B_base + B_windowSize) {
        if (B_ACKstate == 0) {
            B_ACKnum = 999;
        }
        struct pkt packet = make_pkt(message.data, B_next_seqnum, B_ACKnum);
        B_sender_window[B_next_seqnum - B_base] = packet; // Store packet in sender window
        printf("B_output: send packet (seq=%d): %s\n", packet.seqnum, packet.payload);

        tolayer3(1, packet);
        B_ACKstate = 0;
        if (B_base == B_next_seqnum) {
            starttimer(1, B_timer);
        }
        B_next_seqnum++;
    }
    else {
        printf("B_output: window is full!!\n");
    }
}
...
A_timerinterrupt() // Retransmit packets from base to next_seqnum - 1
{
    for (int i = A_base; i < A_next_seqnum; ++i) {
        struct pkt* packet = &A_sender_window[i - A_base];
        printf("A_timerinterrupt: resend packet (seq=%d): %s\n", packet->seqnum, packet->payload);
        tolayer3(0, *packet);
    }
    starttimer(0, A_timer); // Restart timer
...
B_timerinterrupt()
{
    for (int i = B_base; i < B_next_seqnum; ++i) {
        struct pkt* packet = &B_sender_window[i - B_base];
        printf("B_timerinterrupt: resend packet (seq=%d): %s\n", packet->seqnum, packet->payload);
        tolayer3(1, *packet);
    }
    starttimer(1, B_timer); // Restart timer
    printf("B_timerinterrupt: timer + %f\n", B_timer);
}
